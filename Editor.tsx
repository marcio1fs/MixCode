...
import { FileSystemNode, PluginSuggestion, PluginsConfig } from '../types';
...
const Editor: React.FC<EditorProps> = ({ file, onAskAI, onFileContentChange, onRunCode, pluginConfig }) => {
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-gray-900">
        <div className="text-center">
          <Icon name="file" className="w-16 h-16 mx-auto text-gray-600" />
          <p className="mt-2 text-lg text-gray-500">Select a file to begin</p>
          <p className="text-sm text-gray-600">or open the AI Chat to get started.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between bg-gray-800 p-2 border-b border-gray-700 flex-shrink-0">
        ...
        <div className="flex items-center space-x-2">
            {file.language === 'markdown' && (
              <button onClick={() => setIsPreviewMode(!isPreviewMode)} className="flex items-center space-x-1 px-2 py-1 bg-gray-700 hover:bg-gray-600 rounded text-sm text-gray-300">
            ...
            <button
              ...
              className="flex items-center space-x-2 px-3 py-1 bg-green-700 hover:bg-green-600 rounded text-sm text-white disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-gray-600"
            >
              <Icon name="play" className="w-4 h-4"/>
              <span>Run</span>
            </button>
            <button onClick={() => handleAction('explain')} className="flex items-center space-x-1 px-2 py-1 bg-gray-700 hover:bg-gray-600 rounded text-sm text-gray-300" disabled={!file.content || isPreviewMode}>
                <Icon name="sparkles" className="w-4 h-4"/>
                <span>Explain</span>
            </button>
            <button onClick={() => handleAction('refactor')} className="flex items-center space-x-1 px-2 py-1 bg-gray-700 hover:bg-gray-600 rounded text-sm text-gray-300" disabled={!file.content || isPreviewMode}>
                <Icon name="sparkles" className="w-4 h-4"/>
                <span>Refactor</span>
            </button>
            <button onClick={() => handleAction('test')} className="flex items-center space-x-1 px-2 py-1 bg-gray-700 hover:bg-gray-600 rounded text-sm text-gray-300" disabled={!file.content || isPreviewMode}>
                <Icon name="sparkles" className="w-4 h-4"/>
